...
static int ringpu_finite_float(float value);
...
static int ringpu_versioned(uint32_t version, uint32_t size,
...
static int ringpu_slot_const(const RinGpuCore* core, RinGpuHandle handle,
    ...
    if (!handle) return RIN_GPU_ERROR_INVALID_HANDLE;
...
static int ringpu_slot(RinGpuCore* core, RinGpuHandle handle,
                       uint16_t expected_type, uint32_t* index_out,
...
static int ringpu_allocate(RinGpuCore* core, uint16_t type,
                           RinGpuHandle* handle, RinGpuObjectSlot** slot_out) {
...
static void ringpu_release_slot(RinGpuObjectSlot* slot) {
    slot->occupied = 0u;
...
static int ringpu_blend_factor_valid(uint32_t factor) {
    return factor >= RIN_GPU_BLEND_ZERO &&
           factor <= RIN_GPU_BLEND_ONE_MINUS_DESTINATION_COLOR;
}

static int ringpu_blend_source_factor_valid(uint32_t factor) {
    return ringpu_blend_factor_valid(factor) ||
           factor == RIN_GPU_BLEND_SOURCE_ALPHA_SATURATE;
}

static int ringpu_blend_factor_v2_valid(uint32_t factor) {
    return (factor >= RIN_GPU_BLEND_ZERO &&
            factor <= RIN_GPU_BLEND_ONE_MINUS_DESTINATION_COLOR) ||
...
static int ringpu_blend_source_factor_v2_valid(uint32_t factor) {
    return ringpu_blend_factor_v2_valid(factor) ||
           factor == RIN_GPU_BLEND_SOURCE_ALPHA_SATURATE;
}

static int ringpu_blend_constants_valid(uint32_t color_format,
    ...
    /* Float color targets must preserve finite blend constants outside the
...
static int ringpu_blend_operation_valid(uint32_t operation) {
    return operation >= RIN_GPU_BLEND_ADD &&
           operation <= RIN_GPU_BLEND_MAXIMUM;
}

static int ringpu_cull_mode_valid(uint32_t mode) {
    return mode >= RIN_GPU_CULL_NONE && mode <= RIN_GPU_CULL_BACK;
}

static int ringpu_front_face_valid(uint32_t front_face) {
    return front_face == RIN_GPU_FRONT_FACE_COUNTER_CLOCKWISE ||
           front_face == RIN_GPU_FRONT_FACE_CLOCKWISE;
}

static int ringpu_varying_type_valid(uint32_t type) {
    return type == RIN_GPU_VARYING_FLOAT32 ||
           type == RIN_GPU_VARYING_SINT32;
}

static int ringpu_varying_interpolation_valid(uint32_t interpolation) {
...
static int ringpu_vertex_format_valid(uint32_t format) {
    switch (format) {
...
static uint32_t ringpu_vertex_format_bytes(uint32_t format) {
    switch (format) {
    case RIN_GPU_VERTEX_UINT8:
    case RIN_GPU_VERTEX_SINT8:
    case RIN_GPU_VERTEX_UNORM8:
...
static int ringpu_sampler_filter_valid(uint32_t filter) {
    return filter == RIN_GPU_SAMPLER_FILTER_NEAREST ||
           filter == RIN_GPU_SAMPLER_FILTER_LINEAR;
}

static int ringpu_sampler_mip_filter_valid(uint32_t filter) {
    return filter == RIN_GPU_SAMPLER_MIP_FILTER_NONE ||
           ringpu_sampler_filter_valid(filter);
}

static int ringpu_sampler_address_valid(uint32_t address) {
...
static int ringpu_compare_op_valid(uint32_t compare) {
...
static int ringpu_stencil_op_valid(uint32_t operation) {
...
static int ringpu_sampler_desc_valid(const RinGpuSamplerDescV1* desc) {
    if (!desc ||
    ...
    /* Sampling mode and maximum anisotropy are independent state.  In
     * particular, EXT_texture_filter_anisotropic permits nearest spatial or
     * mip filtering together with an anisotropy request; a backend can apply
     * the degree only where its sampling implementation benefits from it. */
    return 1;
}

static int ringpu_build_vertex_layout(
    ...
    if (!explicit_layout) {
        desc->vertex_stride =
            shader_input_count * (uint32_t)sizeof(uint32_t);
...
static int ringpu_build_vertex_layout_bindings(
...
static uint32_t ringpu_shader_interface_location_type(
...
static int ringpu_build_varying_layout(
    ...
    const RinGpuVaryingV1* varyings,
    uint32_t varying_count, RinGpuBackendGraphicsPipelineDescV1* desc) {
    ...
    for (uint32_t component = 0u; component < 4u; component++) {
        if (ringpu_shader_interface_location_type(
    ...
    if ((native_flags & RIN_GPU_GRAPHICS_PIPELINE_NATIVE_POINT_SIZE_OUTPUT) !=
        ...
        if (5u > vertex_info->output_count - position_output_location ||
            ringpu_shader_interface_location_type(
...
static int ringpu_depth_pipeline_valid(uint32_t format, uint32_t compare,
    ...
    return (format == RIN_GPU_FORMAT_D32_FLOAT ||
            format == RIN_GPU_FORMAT_D32_FLOAT_S8_UINT) &&
           ringpu_compare_op_valid(compare) &&
           write_enabled <= 1u;
}

static int ringpu_stencil_face_valid(uint32_t compare,
...
static int ringpu_stencil_pipeline_valid(
...
static uint32_t ringpu_index_format_bytes(uint32_t format) {
...
/* There must be at least one representable unsigned index whose widened
 * signed sum with base_vertex falls inside [0, vertex_count). The backend is
 * still required to check this predicate for every fetched index. */
static int ringpu_base_vertex_has_valid_index(int32_t base_vertex,
...
static int ringpu_image_allocation_size(
    ...
    if (ringpu_depth_stencil_format(desc->format)) {
        ...
        /* Portable Float RGBA (binary16 or binary32) is a sampled, offscreen
         * color attachment with optional CPU upload/readback. A backend still
         * has to accept the descriptor; scanout and storage images remain
         * deliberately absent. */
...
static void ringpu_release_command_references(RinGpuCore* core,
    ...
    for (uint32_t index = 0; index < list->value.command_list.count; index++) {
        ...
        RinGpuObjectSlot* destination = NULL;
        RinGpuObjectSlot* source = NULL;
        uint16_t object_type;
        if (command->type == RIN_GPU_BACKEND_COMMAND_DISPATCH) {
            if (ringpu_slot(core, command->destination,
                            RIN_GPU_OBJECT_COMPUTE_PIPELINE, NULL,
                            &destination) == RIN_GPU_OK &&
                destination->value.compute_pipeline.reference_count != 0u) {
                destination->value.compute_pipeline.reference_count--;
            }
            if (ringpu_slot(core, command->source,
                            RIN_GPU_OBJECT_COMPUTE_BIND_GROUP, NULL,
                            &source) == RIN_GPU_OK &&
                source->value.compute_bind_group.reference_count != 0u) {
                source->value.compute_bind_group.reference_count--;
            }
            continue;
        }
        if (command->type == RIN_GPU_BACKEND_COMMAND_DRAW) {
            if (ringpu_slot(core, command->destination,
                            RIN_GPU_OBJECT_GRAPHICS_PIPELINE, NULL,
                            &destination) == RIN_GPU_OK &&
                destination->value.graphics_pipeline.reference_count != 0u) {
                destination->value.graphics_pipeline.reference_count--;
            }
            if (ringpu_slot(core, command->source, RIN_GPU_OBJECT_IMAGE, NULL,
                            &source) == RIN_GPU_OK &&
                source->value.image.reference_count != 0u) {
                source->value.image.reference_count--;
            }
            ringpu_release_graphics_bind_group_reference(
                core, command->resources);
            continue;
        }
        if (command->type == RIN_GPU_BACKEND_COMMAND_DRAW_VERTICES) {
            ...
            if (ringpu_slot(core,
                            command->value.draw_vertices.vertex_buffer,
                            RIN_GPU_OBJECT_BUFFER, NULL,
                            &vertex_buffer) == RIN_GPU_OK &&
                vertex_buffer->value.buffer.reference_count != 0u) {
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_DRAW_VERTICES_V2) {
            if (ringpu_slot(core, command->destination,
                            RIN_GPU_OBJECT_GRAPHICS_PIPELINE, NULL,
                            &destination) == RIN_GPU_OK &&
                destination->value.graphics_pipeline.reference_count != 0u) {
            ...
            for (uint32_t binding = 0u;
                ...
                if (ringpu_slot(
                        ...
                        command->value.draw_vertices_v2
                            .vertex_buffers[binding].buffer,
                        RIN_GPU_OBJECT_BUFFER, NULL, &vertex_buffer) ==
                        RIN_GPU_OK &&
                    vertex_buffer->value.buffer.reference_count != 0u) {
                    vertex_buffer->value.buffer.reference_count--;
                }
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_DRAW_INDEXED ||
            ...
            RinGpuObjectSlot* vertex_buffer = NULL;
            RinGpuObjectSlot* index_buffer = NULL;
            if (ringpu_slot(core, command->destination,
                            RIN_GPU_OBJECT_GRAPHICS_PIPELINE, NULL,
                            &destination) == RIN_GPU_OK &&
                destination->value.graphics_pipeline.reference_count != 0u) {
            ...
            if (ringpu_slot(core,
                            command->value.draw_indexed.vertex_buffer,
                            RIN_GPU_OBJECT_BUFFER, NULL,
                            &vertex_buffer) == RIN_GPU_OK &&
                vertex_buffer->value.buffer.reference_count != 0u) {
                vertex_buffer->value.buffer.reference_count--;
            ...
            if (ringpu_slot(core,
                            command->value.draw_indexed.index_buffer,
                            RIN_GPU_OBJECT_BUFFER, NULL,
                            &index_buffer) == RIN_GPU_OK &&
                index_buffer->value.buffer.reference_count != 0u) {
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_DRAW_INDEXED_V2) {
            RinGpuObjectSlot* index_buffer = NULL;
            if (ringpu_slot(core, command->destination,
                            RIN_GPU_OBJECT_GRAPHICS_PIPELINE, NULL,
                            &destination) == RIN_GPU_OK &&
            ...
            if (ringpu_slot(core,
                            command->value.draw_indexed_v2.index_buffer,
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_BEGIN_RENDER_PASS) {
            if (ringpu_slot(core, command->destination, RIN_GPU_OBJECT_IMAGE,
                            NULL, &destination) == RIN_GPU_OK &&
                destination->value.image.reference_count != 0u) {
                destination->value.image.reference_count--;
            }
            continue;
        }
        if (command->type == RIN_GPU_BACKEND_COMMAND_BEGIN_RENDER_PASS_MRT) {
            ...
            for (uint32_t color_index = 0u;
                ...
                if ((pass->active_color_mask & (1u << color_index)) == 0u)
                    continue;
                target = pass->color_attachments[color_index].target;
                if (ringpu_slot(core, target, RIN_GPU_OBJECT_IMAGE, NULL,
                                &destination) == RIN_GPU_OK &&
                    destination->value.image.reference_count != 0u) {
                    destination->value.image.reference_count--;
                }
            ...
            if (pass->depth_target != 0u &&
                ringpu_slot(core, pass->depth_target, RIN_GPU_OBJECT_IMAGE,
                            NULL, &source) == RIN_GPU_OK &&
                source->value.image.reference_count != 0u) {
                source->value.image.reference_count--;
            }
            if (pass->stencil_target != 0u &&
                ringpu_slot(core, pass->stencil_target, RIN_GPU_OBJECT_IMAGE,
                            NULL, &source) == RIN_GPU_OK &&
                source->value.image.reference_count != 0u) {
                source->value.image.reference_count--;
            }
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_BEGIN_RENDER_PASS_DEPTH ||
            command->type ==
            ...
            if (command->type ==
                    RIN_GPU_BACKEND_COMMAND_BEGIN_RENDER_PASS_DEPTH_STENCIL &&
                ringpu_slot(core, command->auxiliary, RIN_GPU_OBJECT_IMAGE,
                            NULL, &source) == RIN_GPU_OK &&
                source->value.image.reference_count != 0u) {
                source->value.image.reference_count--;
            }
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_PRESENT) {
            if (ringpu_slot(core, command->destination, RIN_GPU_OBJECT_IMAGE,
                            NULL, &destination) == RIN_GPU_OK &&
                destination->value.image.reference_count != 0u) {
                destination->value.image.reference_count--;
        ...
        if (command->type == RIN_GPU_BACKEND_COMMAND_COPY_BUFFER) {
            object_type = RIN_GPU_OBJECT_BUFFER;
        } else if (command->type == RIN_GPU_BACKEND_COMMAND_COPY_IMAGE ||
                   command->type ==
                       RIN_GPU_BACKEND_COMMAND_TRANSITION_IMAGE) {
            object_type = RIN_GPU_OBJECT_IMAGE;
        } else {
            continue;
        }
        if (ringpu_slot(core, command->destination, object_type, NULL,
                        &destination) == RIN_GPU_OK) {
            uint32_t* references = object_type == RIN_GPU_OBJECT_BUFFER
                ? &destination->value.buffer.reference_count
                : &destination->value.image.reference_count;
            if (*references != 0u) (*references)--;
        }
        if (command->type != RIN_GPU_BACKEND_COMMAND_TRANSITION_IMAGE &&
            ringpu_slot(core, command->source, object_type, NULL,
                        &source) == RIN_GPU_OK) {
            uint32_t* references = object_type == RIN_GPU_OBJECT_BUFFER
                ? &source->value.buffer.reference_count
                : &source->value.image.reference_count;
            if (*references != 0u) (*references)--;
        }
    }
...
static int ringpu_record_command(RinGpuObjectSlot* list,
...
static uint32_t ringpu_image_mip_extent(uint32_t extent,
                                        uint32_t mip_level) {
...
static int ringpu_image_copy_side_valid(
...
static int ringpu_image_upload_source_valid(
    ...
    format_bytes = ringpu_image_format_bytes(desc->format);
    if (format_bytes == 0u ||
        !ringpu_multiply_u64((uint64_t)upload->width, format_bytes,
                             &row_bytes)) {
        return RIN_GPU_ERROR_INVALID_ARGUMENT;
    }
    if (upload->source_row_pitch_bytes == 0u) {
        upload->source_row_pitch_bytes = row_bytes;
    }
    if (upload->source_row_pitch_bytes < row_bytes ||
        !ringpu_multiply_u64(upload->source_row_pitch_bytes,
                             upload->height, &minimum_slice_pitch)) {
        return RIN_GPU_ERROR_INVALID_ARGUMENT;
    }
    if (upload->source_slice_pitch_bytes == 0u) {
        upload->source_slice_pitch_bytes = minimum_slice_pitch;
    }
    if (upload->source_slice_pitch_bytes < minimum_slice_pitch ||
        !ringpu_multiply_u64((uint64_t)(upload->depth - 1u),
                             upload->source_slice_pitch_bytes,
                             &extra_slices) ||
        !ringpu_multiply_u64((uint64_t)(upload->height - 1u),
                             upload->source_row_pitch_bytes, &extra_rows) ||
...
static int ringpu_image_readback_destination_valid(
    ...
    if (!desc || !readback ||
        !ringpu_versioned(readback->abi_version, readback->struct_size,
                          sizeof(*readback)) ||
        readback->flags != 0u || readback->reserved != 0u) {
        return RIN_GPU_ERROR_INVALID_ARGUMENT;
    }
    if (!ringpu_image_copy_side_valid(
    ...
    format_bytes = ringpu_image_format_bytes(desc->format);
    if (format_bytes == 0u ||
        !ringpu_multiply_u64((uint64_t)readback->width, format_bytes,
                             &row_bytes)) {
        return RIN_GPU_ERROR_INVALID_ARGUMENT;
    }
    if (readback->destination_row_pitch_bytes == 0u) {
        readback->destination_row_pitch_bytes = row_bytes;
    }
    if (readback->destination_row_pitch_bytes < row_bytes ||
        !ringpu_multiply_u64(readback->destination_row_pitch_bytes,
                             readback->height, &minimum_slice_pitch)) {
        return RIN_GPU_ERROR_INVALID_ARGUMENT;
    }
    if (readback->destination_slice_pitch_bytes == 0u) {
        readback->destination_slice_pitch_bytes = minimum_slice_pitch;
    }
    if (readback->destination_slice_pitch_bytes < minimum_slice_pitch ||
        !ringpu_multiply_u64((uint64_t)(readback->depth - 1u),
                             readback->destination_slice_pitch_bytes,
                             &extra_slices) ||
        !ringpu_multiply_u64((uint64_t)(readback->height - 1u),
    ...
    required_size = extra_slices + extra_rows + row_bytes;
    return destination_size < required_size ? RIN_GPU_ERROR_BOUNDS : RIN_GPU_OK;
}
...
static int ringpu_image_state_allowed(const RinGpuImageDescV1* desc,
                                      uint32_t state) {
    if (!desc) return 0;
...
static int ringpu_raster_state_valid(const RinGpuRasterStateV1* state) {
    ...
    if (!ringpu_versioned(viewport->abi_version, viewport->struct_size,
        ...
        !ringpu_versioned(scissor->abi_version, scissor->struct_size,
                          sizeof(*scissor)) ||
        scissor->enabled > 1u || scissor->flags != 0u ||
        scissor->reserved0 != 0u || scissor->reserved1 != 0u) {
        return 0;
    }
